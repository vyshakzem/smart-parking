 ...
 */
@WebServlet("/ParkingSlot3Servlet")
public class ParkingSlot3Servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out=response.getWriter();
	     ...
	     ResultSet resultSet;
	     final String SELECT_QUERY = "SELECT * from SlotKarpagam";
	     out.println("<html>");
			out.println("<head>"+"<link rel=\"stylesheet\" href=\"Slot.css\">"+"<title>SMART PARKING SYSTEM</title>");
			out.println("</head>");
			out.println("<body>");
			out.println("<ul>\r\n"
					+ "\r\n"
			...
			out.println("<div class=\"filters\">\r\n"
					...
					+ "  <button  onclick=\"location.href='ParkingSlot1Servlet';\"class=\"filter-option\" data-filter=\"flies\">OTHAKALMANDABAM</button> \r\n"
			...
			try {
				...
				out.println("<form action=\"Booking2.jsp\" method =\"post\">\r\n"
						+ "<h1>Parking Slot</h1>\r\n"
						+ "    <div class=\"parking-lot\">");
				while(resultSet.next()) {
					if(resultSet.getString("Status").equals("0")) {
						String slot= resultSet.getString("Slots");
						out.println(" <div class=\"parking-space free\">\r\n"
								+ "              <button onclick=\"location.href='Booking2.jsp';\" value ='"+slot+"'  name = \"slot\" >" + resultSet.getString("Slots") +"</button>\r\n"
								+ "              </div>");
					}
					else {
						out.println(" <div class=\"parking-space taken\">\r\n"
								+ "              <button disabled>"+ resultSet.getString("Slots") +"</button>\r\n"
								+ "              </div>");
